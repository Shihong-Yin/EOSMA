...
weight = ones(PopSize,dim);% Fitness weight of each slime mold
...
while  it <= MaxIter
    ...
    Ceq_ave=(bestPositions + Ceq2 + Ceq3 + Ceq4)/4;        % Averaged candidate
    ...
    for i=1:PopSize
        if rand<z
            Ceq=C_pool(rn(i,1),:); % Select a solution at random from the equilibrium pool
    ...
    if rand<q
        ...
        CF=(1-it/MaxIter)^(2*it/MaxIter); % Contraction factor
